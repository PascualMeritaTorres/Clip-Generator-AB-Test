...
def load_mp3_timestamps(filepath: str) -> Dict:
    ...
    Returns:
        Dict: Dictionary containing the MP3 timestamps data
    """
    with open(filepath, 'r') as f:
        data = json.load(f)
    assert isinstance(data, list), "Input data must be a list"
    return data[0]  # Get first element since it's a single-element list

def create_word_timestamps(characters: List[str], start_times: List[float]) -> List[Dict[str, str]]:
    """
    ...
    """
    assert len(characters) == len(start_times), "Characters and start times must have same length"
    ...
    for char, time in zip(characters, start_times):
        if char == " ":
            if current_word:
                word = "".join(current_word)
                result.append({
                    "word": word,
                    "timestamp": f"{current_start_time:.3f}"
                })
                current_word = []
                current_start_time = None
        elif char in PUNCTUATION:
            # First add the current word if it exists
            if current_word:
                word = "".join(current_word)
                result.append({
                    "word": word,
            ...
            # Then add the punctuation mark as a separate entry
            result.append({
                "word": char,
                "timestamp": f"{time:.3f}"
            })
...
def process_timestamps(input_filepath: str, output_dir: str = None) -> Tuple[str, str]:
    ...
    # Get the first (and only) audio file data
    audio_file_data = list(data.values())[0]
    
    # Extract characters and start times
    characters = audio_file_data["characters"]
    start_times = audio_file_data["character_start_times_seconds"]
    
    # Create word timestamps
    word_timestamps = create_word_timestamps(characters, start_times)
